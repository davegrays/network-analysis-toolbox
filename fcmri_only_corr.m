function fcmri_only_corr(ROI_info_file,subject_info_file,R_array,focus_group)
...
%     fcmri_corr('22ROIs_control+cblm_info.txt','210subjects_info.txt','22ROIs_210subjects.mat','all')
% B. make threshold dropping movie for adults only:
%     fcmri_corr('22ROIs_control+cblm_info.txt','210subjects_info.txt','22ROIs_210subjects.mat','adults')
%
...
tc_dir='/media/DATA/Resting_State/Amyg_Monkeys/Analyses/unknown';
...
if exist(R_array,'file') == 0
    ...
    for i=1:length(vc_names{1,1}) % for each subject
        for j=1:size(ROI_info{1,1},1) % get each ROI we're interested in
            prefix=[tc_dir '/connectivity_ready_data/' vc_names{1,1}{i,1} '/timecourses/' vc_names{1,1}{i,1} '_Macaque.F99UA1.' char(ROI_info{1,5}(j,1)) ]; %uncomment for DREMS
